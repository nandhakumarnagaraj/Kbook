...
1. **Refund attempt accounting is optimistic.** `RefundService.initiatePartialRefund` adds an amount to `bill.refundAmount` and can set the bill to `refunded` when Easebuzz has only *initiated* the refund. A failed, reversed, or duplicate callback needs a durable per-attempt record so initiated, completed, and failed amounts are distinct. The current bill-level `refundId` also cannot represent multiple partial attempts reliably. The callback hash does not bind its refund ID or amount, so the callback must be matched to a stored attempt before applying financial state. See `RefundService.java`, `EasebuzzPaymentService.java`, and `EasebuzzWebhookService.java`.
...
- **15 — Refund-attempt ledger:** persist one immutable request ID, requested amount, Easebuzz payment ID, refund ID, and lifecycle per attempt; separate initiated and completed totals; reject callbacks that do not match that attempt. Prove partial, repeated, failed, and out-of-order callbacks cannot over-refund or falsely mark a bill complete.
- **16 — Durable work:** persist delayed refunds and post-split jobs, dispatch after commit, retry with stable idempotency IDs, and expose failures for operator reconciliation. Prove restart/retry recovery.
