...
- Deploy workflow: manual on VPS — `git pull` then `./ops/deploy-production.sh`
- Deploy status command: `docker compose --env-file .env -f ops/docker-compose.production.yml ps`
- Merge method: squash
...
- Deploy trigger: on VPS `/var/www/kbook.iadv.cloud`: `git pull && ./ops/deploy-production.sh` (builds image, `up -d postgres server`, backend on `127.0.0.1:8081`, Apache proxies `/api/v1/`)
- Deploy status: `docker compose --env-file .env -f ops/docker-compose.production.yml ps` and `... logs -n 100 server`
