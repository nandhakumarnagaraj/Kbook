...
sealed class BillIntent {
     ...
     * Carries the operator-selected payment mode and split allocation so a
     * restored draft does not fall back to plain UPI with zero split.
     */
    data class DraftForPayment(
    ...
    /**
     * Cancelled sale: an abandoned new-order payment must be representable
     * as a cancelled sale (no payments, no stock), never a silent successful Settle.
     */
    data class Cancelled(val reason: String = "") : BillIntent()
}
...
class BillCreationUseCase @Inject constructor(
    ...
    suspend fun createBill(params: BillCreationParams): BillCreationResult {
        ...
        val bill = BillEntity(
            ...
            partAmount1 = when (val intent = params.intent) {
                ...
                is BillIntent.DraftForPayment -> intent.partAmount1
                else -> "0.0"
            },
            partAmount2 = when (val intent = params.intent) {
                is BillIntent.Settle -> intent.partAmount2
                is BillIntent.DraftForPayment -> intent.partAmount2
                else -> "0.0"
            },
    ...
    private fun resolveStatuses(params: BillCreationParams): Statuses {
        return when (val intent = params.intent) {
            ...
            is BillIntent.Cancelled -> Statuses(
 ...
 */
fun com.khanabook.lite.pos.feature.billing.viewmodel.BillingViewModel.CartItem.toSnapshot(): CartItemSnapshot =
    CartItemSnapshot(
        menuItemId = item.id,
        itemName = item.name,
