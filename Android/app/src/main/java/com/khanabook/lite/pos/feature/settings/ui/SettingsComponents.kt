...
@Composable
internal fun LegacyLogoutSectionUnused(viewModel: com.khanabook.lite.pos.feature.auth.viewmodel.LogoutViewModel) {
    val spacing = KhanaBookTheme.spacing
...
internal fun LegacyAppLockConfigViewUnused(
    onBack: () -> Unit,
    viewModel: com.khanabook.lite.pos.feature.auth.viewmodel.AppLockViewModel = hiltViewModel()
) {
    val context = LocalContext.current
    val spacing = KhanaBookTheme.spacing
    val iconSize = KhanaBookTheme.iconSize
    ...
    LaunchedEffect(setupState) {
        if (setupState is com.khanabook.lite.pos.feature.auth.viewmodel.PinSetupState.Success) {
    ...
    Column(
        ...
        if (isEnabled && setupState is com.khanabook.lite.pos.feature.auth.viewmodel.PinSetupState.Idle) {
            KhanaBookCard(
                modifier = Modifier.fillMaxWidth(),
                colors = CardDefaults.cardColors(containerColor = CardBG),
                shape = KhanaRadii.lg
        ...
        if (setupState is com.khanabook.lite.pos.feature.auth.viewmodel.PinSetupState.Success) {
            Text(
                (setupState as com.khanabook.lite.pos.feature.auth.viewmodel.PinSetupState.Success).message,
                color = SuccessGreen,
                style = MaterialTheme.typography.bodyMedium,
        ...
        when (val state = setupState) {
            is com.khanabook.lite.pos.feature.auth.viewmodel.PinSetupState.EnterNew -> {
                Text(
            ...
            is com.khanabook.lite.pos.feature.auth.viewmodel.PinSetupState.ConfirmNew -> {
                Text(
                    "Confirm your PIN",
                    color = TextLight,
                    style = MaterialTheme.typography.titleSmall,
            ...
            is com.khanabook.lite.pos.feature.auth.viewmodel.PinSetupState.EnterCurrent -> {
                Text(
