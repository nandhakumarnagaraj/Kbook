package com.khanabook.lite.pos.feature.billing.viewmodel
...
import com.khanabook.lite.pos.feature.printing.domain.PrintService
...
import com.khanabook.lite.pos.feature.printing.viewmodel.PrintCoordinator
...
class BillingViewModel @Inject constructor(
    ...
    val printerManager: com.khanabook.lite.pos.feature.printing.domain.BluetoothPrinterManager,
    ...
    private val khanaBookApi: com.khanabook.lite.pos.core.network.KhanaBookApi,
    ...
    val connectionStatus: StateFlow<com.khanabook.lite.pos.feature.sync.domain.ConnectionStatus> =
        networkMonitor.status.stateIn(
            viewModelScope,
            SharingStarted.Eagerly,
            com.khanabook.lite.pos.feature.sync.domain.ConnectionStatus.Unavailable
        )
...
if (!validatePaymentLimits(finalSummary.total, paymentStateManager.paymentMode.value, paymentStateManager.partAmount1.value, paymentStateManager.partAmount2.value)) {
        ...
        orderMutex.withLock {
            ...
            try {
                ...
                if (finalized.outcome ==
                    com.khanabook.lite.pos.feature.billing.data.BillFinalizationOutcome.FINALIZED_NOW
                ) {
                    syncManager.triggerImmediateSync()
                }
    ...
    suspend fun completeOrder(status: PaymentStatus, cancelReason: String = ""): Boolean = withContext(Dispatchers.IO) {
        orderMutex.withLock {
            if (!permissionManager.hasPermission(com.khanabook.lite.pos.feature.staff.domain.PermissionManager.BILLING_CREATE)) {
                _blockedPermission.value = BlockedPermission(
                    key = com.khanabook.lite.pos.feature.staff.domain.PermissionManager.BILLING_CREATE,
                    displayName = permissionManager.getDisplayName(com.khanabook.lite.pos.feature.staff.domain.PermissionManager.BILLING_CREATE)
                )
                return@withLock false
            }
            if (cartManager.currentItems.isEmpty()) {
            ...
            try {
                ...
                if (inserted != null && status == PaymentStatus.SUCCESS) {
                    try {
                        com.khanabook.lite.pos.feature.printing.domain.PrintService.startPrintJob(
                            context = appContext,
    ...
    private suspend fun finalizePaymentRecovery(
        billId: Long,
        finalize: suspend () -> com.khanabook.lite.pos.feature.billing.data.BillFinalizationResult
    ): Boolean = withContext(Dispatchers.IO) {
        orderMutex.withLock {
            _isLoading.value = true
            try {
    ...
    private fun describeSyncFailure(error: Throwable?): String {
        ...
        return when (error) {
            ...
            is com.khanabook.lite.pos.core.util.SyncConflictException ->
