package com.khanabook.lite.pos.feature.menu.viewmodel
...
class MenuViewModel @Inject constructor(
    ...
    fun deleteItemPhoto(
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                com.khanabook.lite.pos.core.designsystem.KhanaToast.show("Dish photo removed", com.khanabook.lite.pos.core.designsystem.ToastKind.Success)
                onDeleted()
            } catch (e: Exception) {
                val msg = com.khanabook.lite.pos.core.util.UserMessageSanitizer.sanitize(e, "Failed to remove photo.")
                com.khanabook.lite.pos.core.designsystem.KhanaToast.show(msg, com.khanabook.lite.pos.core.designsystem.ToastKind.Error)
            } finally {
                _isPhotoUploading.value = false
            }
        ...
        if (isPdf) {
            ...
            viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
                try {
                    ...
                    withContext(kotlinx.coroutines.Dispatchers.Main) {
                        _ocrImportUiState.update { it.copy(isProcessing = false, error = com.khanabook.lite.pos.core.util.UserMessageSanitizer.sanitize(e, "Error loading image. Please try again.")) }
                    }
                }
            }
        } else {
        ...
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            try {
                ...
                for (i in 0 until pageCount) {
                    ...
                    val pageDrafts = com.khanabook.lite.pos.feature.menu.domain.OcrSpatialParser.parse(visionText)
                    if (BuildConfig.DEBUG) {
                        Log.d(
                            ocrDebugTag,
                ...
                withContext(kotlinx.coroutines.Dispatchers.Main) {
                    _ocrImportUiState.update { it.copy(
                        isProcessing = false,
                        error = com.khanabook.lite.pos.core.util.UserMessageSanitizer.sanitize(e, "Failed to process PDF. Please try again.")
                    )}
                }
            }
        }
    ...
    private fun processSpatialVisionText(visionText: com.google.mlkit.vision.text.Text) {
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.Default) {
            val drafts = com.khanabook.lite.pos.feature.menu.domain.OcrSpatialParser.parse(visionText)
        ...
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            ...
            try {
                ...
                withContext(kotlinx.coroutines.Dispatchers.Main) {
                    _ocrImportUiState.update { it.copy(
                        isProcessing = false, 
                        error = com.khanabook.lite.pos.core.util.UserMessageSanitizer.sanitize(e, "Failed to save. Please try again.")
                    )}
                }
            }
        }
