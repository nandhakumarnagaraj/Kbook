...
class MarketplaceWebhookControllerTest {
    ...
    void setup() {
        ...
        lenient().when(profileRepo.findAllByIsDeletedFalseOrderByUpdatedAtDesc()).thenReturn(Collections.emptyList());
    }
    ...
    void webhook_duplicateOrder_shouldReturnDuplicate() {
        ...
        // Service deduplicates internally and returns existing order
        when(marketplaceOrderService.createOrder(any())).thenReturn(existing);

        ResponseEntity<Map<String, Object>> response = callSwiggy(urbanPiperPayload("swiggy", null));

        assertEquals(200, response.getStatusCode().value());
        // v3 controller is idempotent: always returns "received" 
        assertEquals("received", response.getBody().get("status"));
        assertEquals(1L, response.getBody().get("order_id"));
    }
