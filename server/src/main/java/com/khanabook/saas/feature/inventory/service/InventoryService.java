...
public class InventoryService {
	...
	private final com.khanabook.saas.feature.inventory.data.PurchaseOrderRepository poRepository;
	...
	public InventoryService(RawMaterialRepository rawMaterialRepository,
							...
							com.khanabook.saas.feature.inventory.data.PurchaseOrderRepository poRepository) {
     ...
     */
    @Transactional
    public com.khanabook.saas.feature.inventory.data.PurchaseOrder createPurchaseOrder(
            Long restaurantId, Long vendorId, String note,
            List<com.khanabook.saas.feature.inventory.data.PurchaseOrderDtos.PoLine> lines, boolean sendNow) {
        if (lines == null || lines.isEmpty()) {
            throw new IllegalArgumentException("At least one line is required");
        }
        var po = new com.khanabook.saas.feature.inventory.data.PurchaseOrder();
        ...
        po.setCreatedAt(now);
        po.setUpdatedAt(now);
        po.setStatus(sendNow ? com.khanabook.saas.feature.inventory.data.PurchaseOrder.STATUS_SENT
                : com.khanabook.saas.feature.inventory.data.PurchaseOrder.STATUS_DRAFT);
        for (var line : lines) {
            ...
            var item = new com.khanabook.saas.feature.inventory.data.PurchaseOrderItem();
    ...
    public List<RawMaterial> receivePurchaseOrder(Long restaurantId, Long poId,
        ...
        if (!com.khanabook.saas.feature.inventory.data.PurchaseOrder.STATUS_SENT.equals(po.getStatus())
                && !com.khanabook.saas.feature.inventory.data.PurchaseOrder.STATUS_DRAFT.equals(po.getStatus())) {
        ...
        po.setStatus(com.khanabook.saas.feature.inventory.data.PurchaseOrder.STATUS_RECEIVED);
    ...
    public void cancelPurchaseOrder(Long restaurantId, Long poId) {
        var po = poRepository.findById(poId)
                .filter(p -> p.getRestaurantId().equals(restaurantId))
                .orElseThrow(() -> new IllegalArgumentException("PO not found"));
        if (com.khanabook.saas.feature.inventory.data.PurchaseOrder.STATUS_RECEIVED.equals(po.getStatus())) {
            throw new IllegalStateException("Received POs cannot be cancelled");
        }
        po.setStatus(com.khanabook.saas.feature.inventory.data.PurchaseOrder.STATUS_CANCELLED);
        po.setUpdatedAt(System.currentTimeMillis());
        poRepository.save(po);
    }
