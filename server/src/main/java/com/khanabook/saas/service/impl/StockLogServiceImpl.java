...
public class StockLogServiceImpl implements StockLogService {
	...
	public PushSyncResponse pushData(Long tenantId, List<StockLog> payload) {
		...
		while (iterator.hasNext()) {
			...
			if (stockLog.getServerMenuItemId() == null && stockLog.getMenuItemId() != null) {
				...
				if (menuItem.isPresent()) {
					...
					if (serverMenuItem.isPresent() && serverMenuItem.get().getRestaurantId().equals(tenantId)) {
						...
						menuItemRepository.findByRestaurantIdAndLocalIdIn(tenantId, List.of(stockLog.getMenuItemId()))
								.stream().findFirst()
								.ifPresent(m -> stockLog.setServerMenuItemId(m.getId()));
					}
				}
			}
			...
			if (stockLog.getServerMenuItemId() == null && stockLog.getMenuItemId() != null) {
				...
				addFailure(failedLocalIds, failedReasons, stockLog.getLocalId(), "Stock log menu item could not be resolved");
				iterator.remove();
				continue;
			}
			...
			if (stockLog.getServerVariantId() == null && stockLog.getVariantId() != null
				...
				if (variant.isPresent()) {
					...
					if (serverVariant.isPresent() && serverVariant.get().getRestaurantId().equals(tenantId)) {
						...
						itemVariantRepository.findByRestaurantIdAndLocalIdIn(tenantId, List.of(stockLog.getVariantId()))
								.stream().findFirst()
								.ifPresent(v -> stockLog.setServerVariantId(v.getId()));
					}
				}
			}
		}
	...
	@Override
	@Transactional(readOnly = true)
	public org.springframework.data.domain.Page<StockLog> pullData(Long tenantId, Long lastSyncTimestamp, String deviceId, boolean ignoreDeviceId, org.springframework.data.domain.Pageable pageable) {
		if (ignoreDeviceId) {
