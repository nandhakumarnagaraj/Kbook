...
public class ZomatoApiClient {
    ...
    @Value("${zomato.api-base-url:https://api.zomato.com}")
    ...
    private JsonNode exchange(String path, String apiKey, HttpMethod method, String body) {
        ...
        if (!response.getStatusCode().is2xxSuccessful() || response.getBody() == null) {
            throw new IllegalStateException("Zomato API request failed: " + response.getStatusCode());
        }

        try {
            return objectMapper.readTree(response.getBody());
        } catch (Exception e) {
            throw new IllegalStateException("Unable to parse Zomato response", e);
        }
    }
}
