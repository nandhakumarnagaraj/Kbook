...
public class CryptoService {
    ...
    @PostConstruct
    void validateSecret() {
        if (rawSecret == null || rawSecret.isBlank() || rawSecret.contains("${")) {
            throw new IllegalStateException(
                "PAYMENT_CRYPTO_SECRET environment variable is not set. " +
                "Set it before starting the application (min 32 bytes).");
        }
    }
