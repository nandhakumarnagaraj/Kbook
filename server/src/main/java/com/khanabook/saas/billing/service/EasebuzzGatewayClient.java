...
public class EasebuzzGatewayClient {
    ...
    public TransactionLookup fetchTransaction(
            String environment,
            String merchantKey,
            String txnId,
            String amount,
        ...
        JsonNode json = postForm(dashboardBaseUrlFor(environment) + "/transaction/v1/retrieve", form);
        JsonNode data = json.path("data");
        return TransactionLookup.builder()
                .rawPayload(json.toString())
                .apiStatus(stringValue(json, "status"))
                .gatewayStatus(firstNonBlank(
                        stringValue(data, "status"),
                        stringValue(json, "transaction_status"),
                        stringValue(json, "payment_status")))
                .gatewayPaymentId(firstNonBlank(
                        stringValue(data, "easepayid"),
                        stringValue(data, "easebuzz_id"),
                        stringValue(data, "id"),
                        stringValue(json, "easepayid")))
    ...
    @Getter
    @Builder
    public static class TransactionLookup {
        private String rawPayload;
        private String apiStatus;
