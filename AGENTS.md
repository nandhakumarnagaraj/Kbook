...
- **Responsive typography tiers**: `KhanaBookTheme.layout.typeScaleTier` (CompactPhone/MediumPhone/LargePhone/Tablet) is resolved from the current window width+height (never device model). `MaterialTheme.typography` is already the tiered variant app-wide — do NOT hand-scale `fontSize` per screen; add per-slot tiers in `Type.kt` only if a slot needs a different curve. Tier card tokens: `layout.cardPaddingHorizontal/cardPaddingVertical/primaryCardVertical/actionIconContainerSize/actionIconSize/primaryIconContainerSize/primaryIconSize/homeActionColumns`
- **HomeScreen density rules (final, do not regress)**: no scrollbar and all 5 actions (Create New Order/Active Orders/Find Bill/Reprint KOT/Call Customer) always visible; no large fixed `Spacer()`s; distribute leftover height via section-gap growth (height tiers in `layout.sectionSpacing`) or `Arrangement.SpaceBetween` at the top-level column — never `weight(1f)`/`fillMaxHeight()` on individual cards (content-driven sizing only); cap content width with `layout.maxContentWidth`; extra horizontal padding via `layout.contentPadding`
